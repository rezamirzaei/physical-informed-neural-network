...
from physical_informed_neural_network.data import (
...
from physical_informed_neural_network.model import BurgersPINN
from physical_informed_neural_network.physics import BurgersResidual
from physical_informed_neural_network.plotting import (
    apply_plot_style,
    plot_comparison,
    plot_loss_history,
...
from physical_informed_neural_network.schemas import (
    ErrorMetrics,
    ExperimentSummary,
    ObservationSet,
    ReferenceSolution,
...
from physical_informed_neural_network.training import PINNTrainer
from physical_informed_neural_network.utils import ensure_directory, select_device, set_global_seed
...
def _save_artifacts(
    ...
    import torch
    from physical_informed_neural_network.config import PDEConfig

    pde_bounds = PDEConfig(
        viscosity=summary.viscosity,
        x_min=summary.domain_x[0], x_max=summary.domain_x[1],
        t_min=summary.domain_t[0], t_max=summary.domain_t[1],
    )
    interior_pts = sample_interior_collocation(pde_bounds, 5000, seed=999)
...
def run_experiment(config: ProjectConfig) -> ExperimentArtifacts:
    ...
    from physical_informed_neural_network.data import _cole_hopf_solution

    u_ref_pred = _cole_hopf_solution(x_pred, t_pred, config.pde.viscosity)
    ...
    if config.artifacts.save_artifacts:
        ...
        (config.artifacts.output_dir / "summary.json").write_text(
